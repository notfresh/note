...
最长递增子序列 递增序列 leet300 这是错的 最长

```python
class Solution:
    def findLengthOfLCIS(self, nums: List[int]) -> int:
        # 这是一个经典的动态规划问题
        # 我们用数组dp来记录相关信息，dp[i]表示在0-i(包括i所在的元素)的子序列中的递增子序列的长度
        # 数组 1，2，1对应的dp是 [1,2,1]
        # 数组 [3,2,1]对应的dp是[1,1,1]
        # 数组 [1,2,3] 对应的dp是 [1,2,3]
        # 数组 [1,1,1] 对应的dp是 [1,1,1]
        # 数组 [] 对应的dp是 []
        # [10,9,2,5,3,7,101,18]
...
第一，动态规划需要的空间复杂度越小，代码就越通俗易懂，如果非要压缩空间复杂度，那么代码就比较晦涩
